...
import { generateRandomEvents as generateEvents } from "@/lib/event-utils";
...
export function useEvents(getCurrentTime?: () => number, imminentThreshold: number = 5) {
  ...
  useEffect(() => {
    ...
    if (cacheEnabled && cachedData) {
      try {
        ...
        if (data.events && Array.isArray(data.events)) {
          console.log('Restoring events:', data.events);
    ...
    console.log('Initializing fresh main event');
      ...
      setEvents(prevEvents => {
        ...
        const updatedEvents = uniqueEvents.map(event => {
          if (event.status === 'upcoming') {
            ...
            // Event completes at: mainEventTimestamp - (event.timeToEvent * 1000)
  ...
  // Sort events by timeToEvent (main event at bottom, oldest to newest)
  const sortedEvents = [...events].sort((a, b) => {
    return b.timeToEvent - a.timeToEvent;
  });
